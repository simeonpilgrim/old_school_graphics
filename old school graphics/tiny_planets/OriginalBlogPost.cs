...
{
    internal class OriginalBlogPost
    {
        public static void OriginalBlogPostSteps()
        {
            ...
            OriginalGifBuilder(raw, @"C:\temp\tinyplanet-out_D.gif", 100, Bend_D);
            ...
            OriginalGifBuilder(raw, @"C:\temp\tinyplanet-out_H.gif", 100, Bend_H);
        }

        static void OriginalGifBuilder(Raw src, string outfileName, int steps, Func<Raw, int, int, Raw> bender)

        {
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                Program.Log($"frame {i} {sw.ElapsedMilliseconds} ms");
